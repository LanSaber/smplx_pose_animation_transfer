...
class ResNetRegression(nn.Module):
    def __init__(self, input_size=51, output_size=9):
...
def main():
    ...
    with gzip.open(os.path.join("data", "labels_clean.all"), 'rb') as f1:
        with gzip.open(os.path.join("data", "labels_joints.all"), 'rb') as f2:
            pose_dicts = pickle.load(f1)
            joint_dicts = pickle.load(f2)
            ...
            for id in range(len(keys)):
                ...
                joints = joint_dicts[keys[id]]

                assert data["smplx_body_pose"].shape[0] == total_frames
                ...
                for i in range(total_frames):
                    ...
                    for output in outputs:
                        ...
                        left_wrist_gt_pos = torch.from_numpy(joints[i][20] - joints[i][0]).to(device)
                        right_wrist_gt_pos = torch.from_numpy(joints[i][21] - joints[i][0]).to(device)
                        left_wrist_gt_pos = torch.unsqueeze(left_wrist_gt_pos, dim=0)
                        right_wrist_gt_pos = torch.unsqueeze(right_wrist_gt_pos, dim=0)

                        if output["is_right"] == 0.0:
                            output_tensor = left_hand_regressor(input_tensor)
                            loss = loss_fn(output_tensor[:, 0:6], left_wrist_gt_6d) + loss_fn(output_tensor[:, 6:9], left_wrist_gt_pos)
                            ...
                            loss = loss_fn(output_tensor[:, 0:6], right_wrist_gt_6d) + loss_fn(output_tensor[:, 6:9], right_wrist_gt_pos)
